...
coordenadas = sapply(listado, function(x) x[3])        # Me quedo solo con el 3er tipo de coordenada
coordenadas = gsub('[^0-9,.,-]','', vel_vec)           # Elimino los caracteres que no quiero utilizar
...
for (i in 1:N){
  for (j in 1:cant_aeropuertos){
    if (datos$Origen.OACI[i] == aeropuertos$ICAO[j]) {
        datos$ciudad_origen[i] = aeropuertos$`City served`[j] 
